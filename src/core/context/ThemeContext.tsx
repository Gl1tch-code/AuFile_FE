...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--primary-text", 
      theme === THEME_VALUES.DARK ? '#FFFFFF' : '#1A1E26');
    document.documentElement.style.setProperty("--secondary-text", 
      theme === THEME_VALUES.DARK ? '#B3C2E1' : '#2E3646');
      document.documentElement.style.setProperty("--primary-bg-app", 
        theme === THEME_VALUES.DARK ? '#2E3646' : '#B3C2E1');
      
  }, [theme])

  const toggleTheme = () => {
    setTheme((prevTheme) => (prevTheme === THEME_VALUES.LIGHT ? THEME_VALUES.DARK : THEME_VALUES.LIGHT));
  };
